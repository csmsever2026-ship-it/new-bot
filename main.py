...
async def main():
    ...
    async def handler(event):
        ...
        print(f"[EVENT] Текущее время МСК: {datetime.now(timezone(timedelta(hours=3))).strftime('%H:%M:%S')}")
...
if __name__ == "__main__":
    ...
    asyncio.run(main())# Этот файл запускается на Railway и пересылает новые посты
# из каналов @vedexx_news, @customs_rf, @oVEDinfo в @clr_group_expert
# Только в рабочее время: 09:00 – 21:00 по Москве (MSK, UTC+3)
